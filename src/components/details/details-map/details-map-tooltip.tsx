...
const getChildren = (
    ...
    const pushIfNotNullArray = (element : JSX.Element[]|null|undefined) => {
        if (element === undefined || element === null)
            return
        element.forEach( e => children.push(e) )
    }
...
// --- Player Tile ---
// -------------------
const getPlayerTile = (playerTile : svg_PlayerType|undefined, missionData : MissionDataType) => {
...
const getStronghold = (
    ...
    let index = strongholdArray[0];
    let stronghold = strongholdArray[1];

    // Make sure Stronghold should be shown
    let show = (missionData.strongholds[index] !== undefined) ? missionData.strongholds[index].appear : undefined;
...
// --- Bases ---
// -------------
const getBase = (bases : [number, BaseDataType]|undefined, missionData : MissionDataType) => {

    if ( bases === undefined || bases[0] === undefined || bases[1] === undefined )
        return null;

    var base : BaseDataType|undefined = bases[1];
    let index = bases[0];
...
// --- Chests ---
// ---------------
const getChest = (chest : svg_ChestType|undefined) => {
    if ( chest === undefined )
        return null;
...
// --- Pots ---
// ------------
const getPots = (pot : PotDataType|undefined) => {
    if ( pot === undefined )
        return null;
...
// -------------
const getUnits = (
        units : Dictionary<UnitDataType>|undefined, 
        battleTable : React.RefObject<any>|undefined, 
        missionTable: React.RefObject<any>|undefined,
        missionText: React.RefObject<Dictionary<TextRefType>>,
        missionData : MissionDataType,  
    ...
    children = Object.entries(units).map( ([key, unit]) => {
